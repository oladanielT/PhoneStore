...
const trustIcons = [ShieldCheck, Truck, MapPin]
...
export function Hero() {
  ...
  const prev = useCallback(() => {
    setCurrent(
      (prev) =>
        (prev - 1 + hero.carouselImages.length) % hero.carouselImages.length,
    )
  }, [hero.carouselImages.length])
  ...
  const hotDealLines = hero.hotDealTitle.split('\n')
          ...
          <div className="absolute -top-6 -left-6 hidden animate-bounce-slow rounded-2xl border bg-background p-4 shadow-xl sm:block z-20">
            <div className="flex flex-col gap-1">
              ...
              <p className="font-heading text-lg font-black text-primary leading-tight">
                {hotDealLines.map((line, i) => (
                  <span key={i}>
                    {line}
                    {i < hotDealLines.length - 1 && <br />}
                  </span>
                ))}
              </p>
              <p className="text-xs font-medium text-muted-foreground">
                {hero.hotDealSubtitle}
              </p>
            </div>
          </div>
        </div>
